    ...
    public PrintWriter actionSaveGenPage() throws IOException, ServletException {
        ...
        if (ac != null) {
            ...
            if (fileJson != null && fileXml != null && fileXsl != null && javaCode != null && javaCode != "" && path_xsl != null && path_xsl != "" && path_class != null && path_class != "") {
                ...
                if (FileHelper.save(path_class, ac.getPage() + ".java", partsJavaCode[0] + "*/")
                        && // save model
                        FileHelper.save(path_class, ac.getPage() + "View.java", "/*" + partsJavaCode[1] + "*/")
                        && // save view
                        FileHelper.save(path_class, ac.getPage() + "Controller.java", "/*" + partsJavaCode[2])
                        && // save controller
                        FileHelper.save(path_xsl, ac.getPage() + ".xml", fileXml)
                        && // save xml
                        FileHelper.save(path_xsl, ac.getPage() + ".xsl", fileXsl)
                        && // save xsl
                        FileHelper.save(path_xsl, ac.getPage() + ".json", fileJson)
                        && // save json
                        CompilerHelper.compile(path_class, ac.getPage() + ".java")
                        && //Compile model
                        CompilerHelper.compile(path_class, ac.getPage() + "View.java")
                        && //Compile controller
