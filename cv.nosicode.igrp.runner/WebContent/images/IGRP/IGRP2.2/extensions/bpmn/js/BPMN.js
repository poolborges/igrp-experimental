...
function init() {
  ...
  // Contexto menu personalizado
  myDiagram.contextMenu = $(go.Adornment);
  var cxElementEvent = document.getElementById("ctx-bpnm-event");
  var cxElementLine = document.getElementById("ctx-bpnm-line");

  cxElementEvent.addEventListener("contextmenu", function(e) { e.preventDefault(); return false; }, false);
  cxElementEvent.addEventListener("blur", function(e) { cxMenu.stopTool(); }, false);

  cxElementLine.addEventListener("contextmenu", function(e) { e.preventDefault(); return false; }, false);
  cxElementLine.addEventListener("blur", function(e) { cxMenu.stopTool(); }, false);

   var cxTool = myDiagram.toolManager.contextMenuTool;
   var cxElement = cxElementEvent, vActivity = 'activity';
   ...
   // try{
      if (vActivity != 'activity') {
        var ctx_child = cxElement.getElementsByClassName("activity").length;
        for (var i = 0; i < ctx_child; i++) {
          cxElement.getElementsByClassName("activity")[i].style.display = "none";
        }
      }
        
        cxElement.style.display = "block";
        var mousePt = diagram.lastInput.viewPoint;
        cxElement.style.left = (mousePt.x + 365) + "px";
   ...
   cxTool.hideContextMenu = function() {
    ...
    var ctx_child = cxElement.getElementsByClassName("activity").length;
      for (var i = 0; i < ctx_child; i++) {
        cxElement.getElementsByClassName("activity")[i].style.display = "block";
      }
  }
  ...
  var activityNodeMenu =
       $(go.Adornment, "Vertical",
         ...
         $("ContextMenuButton",
             $(go.TextBlock, "Link", { margin: 3 }),
             { click: function(e, obj) {
              openUrl(obj.part.data);
             } })
         );

function openUrl(obj){
  var vParam = obj.key?"p_key="+obj.key:"p_from="+obj.from+"&p_to="+obj.to;
  var vUrl = IGRP_getUrl(vUrlGlob)+vParam;
  IGRP_h_popupCtx({pUrl:vUrl,pWin:'IGRP_bpmn'});
}
  ...
  var groupTemplateForPalette = 
    $(go.Group, "Vertical",
      $(go.TextBlock,         // group title
        { alignment: go.Spot.Center, font: "Bold 12pt Sans-Serif",
         stroke:"rgba(41, 68, 96, 1)", margin:10},
      ...
      $(go.Panel, "Auto",
        ...
        $(go.Placeholder,    // represents the area of all member parts,
          { padding: 4})  // with some extra padding around them
      )
    );
  ...
  palGroupTemplateMap.add("OfGroups", groupTemplateForPalette);
      ...
      { contextMenu:
          $(go.Adornment, "Vertical",
            ...
            $("ContextMenuButton",
              $(go.TextBlock, "Link"),
              // in the click event handler, the obj.part is the Adornment; its adornedObject is the port
              { click: function(e, obj) {openUrl(obj.part.data); } })
           ),
  ...
  var level1 = $(go.GraphLinksModel,
    {
      nodeDataArray: [
        /*{key: "Activity Nodes", isGroup: true, category:"OfGroups"},
        {key: "Event Nodes", isGroup: true, category:"OfGroups" },
        {key: "Gateway Nodes", isGroup: true, category:"OfGroups" },
        {key: "Process Nodes", isGroup: true, category:"OfGroups" },*/
        /*{
          key: 701,
          category: "annotation",
          text: "note",
      ...
      /*{
       ...
       },
       {
        ...
        { key: 801, loc: "0 0", text: "Subprocess", isGroup: true, isSubProcess: true, category: "subprocess", taskType: 0, group: "Process Nodes" },
  ...
  /*var level2 = [
    { key: 1, category: "activity", taskType: 1, text: "Message", item: "Message Task", group:"Activity Nodes" },
    { key: 2, category: "activity", taskType: 2, isCompensation: true, text: "User", item: "User Task",
      isCall: true, isLoop: true, isParallel: true, isSequential: true, group:"Activity Nodes"
    },
    { key: 811, loc: "0 0", text: "Adhoc\nSubprocess", isGroup: true, isSubProcess: true, category: "subprocess", isAdHoc: true, taskType: 0, group: "Process Nodes" },
    ...
    { key: 821, loc: "0 0", text: "Transactional\nSubprocess", isGroup: true, isSubProcess: true, category: "subprocess", isTransaction: true, taskType: 0, group: "Process Nodes" },
    ...
    { key: 831, loc: "0 0", text: "Call Subprocess", isGroup: true, isSubProcess: true, category: "subprocess", isCall: true, taskType: 0, group: "Process Nodes" },
  ...
  /*var debugPalette = [
    { key: 101, category: "event", eventType: 4, eventDimension: 8, text: "Escalation", item: "BpmnEventEscalation", group:"Event Nodes" },
    { key: 102, category: "event", eventType: 5, eventDimension: 5, text: "Conditional", item: "BpmnEventConditional", group:"Event Nodes" },
    { key: 105, category: "event", eventType: 6, eventDimension: 4, text: "OffPage", item: "BpmnEventOffPage", group:"Event Nodes" },
    { key: 106, category: "event", eventType: 6, eventDimension: 7, text: "OffPage", item: "BpmnEventOffPage", group:"Event Nodes" },
    { key: 107, category: "event", eventType: 7, eventDimension: 8, text: "Error", item: "BpmnEventError", group:"Event Nodes" },
    { key: 108, category: "event", eventType: 8, eventDimension: 5, text: "Cancel", item: "BpmnEventCancel", group:"Event Nodes" },
    { key: 109, category: "event", eventType: 9, eventDimension: 5, text: "Compensation", item: "BpmnEventCompensation", group:"Event Nodes"},
    { key: 110, category: "event", eventType: 10, eventDimension: 8, text: "Signal", item: "Signal", group:"Event Nodes" },
    { key: 111, category: "event", eventType: 11, eventDimension: 1, text: "Multiple", item: "Multiple", group:"Event Nodes" },
    { key: 112, category: "event", eventType: 12, eventDimension: 1, text: "Parallel", item: "Parallel", group:"Event Nodes" },
    // activity nodes
    { key: 3, category: "activity", taskType: 3, isAdHoc: true, text: "Manual", item: "Manual Task", group:"Activity Nodes" },
    { key: 4, category: "activity", taskType: 4, isSequential: true, text: "Script", item: "Script Task", group:"Activity Nodes" },
    { key: 5, category: "activity", taskType: 5, isParallel: true, text: "Send Msg", item: "Send Msg Task", group:"Activity Nodes" },
    { key: 6, category: "activity", taskType: 6, isLoop: true, isSubProcess: true, isTransaction: true, text: "Service", item: "service task", group:"Activity Nodes" },
    { key: 7, category: "activity", taskType: 7, isSubProcess: true, text: "Business Rule", item: "Business Rule Task", group:"Activity Nodes" },
    // gateway nodes
    { key: 202, category: "gateway", gatewayType: 2, text: "Inclusive", group:"Gateway Nodes" },
    { key: 203, category: "gateway", gatewayType: 3, text: "Complex", group:"Gateway Nodes" }
  ];  // end nodeDataArray
...
function addActivityNodeBoundaryEvent(evType) {
  ...
  myDiagram.selection.each(function(node) {
    ...
    if (node.data.category === "activity" || node.data.category === "subprocess") {
      ...
      var arr = node.data["boundaryEventArray"], vNevType = false;
      //ADD EDSON
      if (arr) {
...
function saveModel() {
  ...
  var nodeData = JSON.stringify(node.nodeDataArray);
  var linkData = JSON.stringify(node.linkDataArray);
