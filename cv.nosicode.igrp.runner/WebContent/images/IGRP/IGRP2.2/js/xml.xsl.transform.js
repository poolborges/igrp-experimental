...
$.fn.XMLTransform = function(params) {
	var element 	 = this;
 	var xml 		 = null;
 	var xsl 		 = null;
 	var loader 		 = !params.loading ? true : params.loading;
 		...
 		try{
	 		if(xml && xsl){
	 			...
	 			var isIE11    = window.navigator.userAgent.match(/rv:11.0/i),
				 	isIE      = window.ActiveXObject  || isIE11 ? true : false,
				 	callback  = isIE ? IETransform : Transform;
 	...
 	var done = function(content){
 		element.removeClass(loadingClass).removeAttr('style');
 	...
 	var loadTemplate = function(p){
 		...
 		if(i < p.includes.length){
 			//var includeFile = p.includes[i];
 			var includeFile = tmplSplitter+p.includes[i];
 	...
 	var getIncludesArr = function(doc){
 		var rtn = [];
 		...
 		$.each($(doc).find('*'),function(){
 			...
 			//	console.log(href);
 				var name = href.indexOf(tmplSplitter) != -1 ? href.split(tmplSplitter)[1] : href;
 		...
 		try{
			...
			xmldoc = xml;
 			...
 			if ((xsldoc.parseError && xsldoc.parseError.errorCode != 0) || (xmldoc.parseError && xmldoc.parseError.errorCode != 0)) {
 				...
 				xslTemlate = new ActiveXObject("Msxml2.XSLTemplate.6.0");
 				
 				xslTemlate.stylesheet = xsldoc;
 				
 				xslproc = xslTemlate.createProcessor();
