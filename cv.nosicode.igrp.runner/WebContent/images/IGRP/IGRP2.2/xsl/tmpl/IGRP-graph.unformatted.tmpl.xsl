<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template name="highcharts">
	...
	$(function () {
		...
		var vX = '', vY = '';
        $('#id-'+id).highcharts({
			//tipo[area,scatter,line,bar,column,spline,stackedarea,stackedcolumn,stackedpercentarea,stackedpercentcolumn,stackedgroupedcolumn]
			chart:{type: _type},
      title:{text: '', x: -20}, //center
      subtitle:{text: '', x: -20},
      xAxis:{
        categories:[<xsl:for-each select="$grafico/label/col[position()!=1]">
      ...
      tooltip: {valueSuffix: '',
        formatter:function(){
          vX = this.x; vY = this.y;
          return '<span style="color:'+this.point.color+'">\u25CF</span><b> '+ vX + '</b><br/><b>' +
              this.series.name + ':</b> ' +vY
              <xsl:if test="$stacking = 'percent'">
               + ' - ('+ this.percentage.toFixed(1)+' %)'
              </xsl:if>
            <xsl:if test="$subtipo != '' and $subtipo = 'column' and $grouped = 'false'">
			...
			plotOptions:{ series:{cursor:'pointer',
				events:{
					click:function(e){
            var vZ = this.name;
            IGRP_graphUrlOpen({pUrl:"<xsl:value-of select='$url'/>",pX:vX,pY:vY,pZ:vZ});
						}
					}
				}
        <xsl:if test="$subtipo != '' and $subtipo = 'column'">
          ,
          column: {stacking:"<xsl:value-of select='$stacking'/>",
        ...
        <xsl:if test="$subtipo != '' and $subtipo = 'area'">
          ,
          area: {stacking:"<xsl:value-of select='$stacking'/>",lineColor: '#666666',lineWidth: 1,
    ...
    $('#id-'+id).highcharts({
      ...
      tooltip: {pointFormat: '{series.name}: <b>{point.percentage:.1f}%</b>',
        formatter: function(){
          ...
          return '<span style="color:'+this.point.color+'">\u25CF</span> '+vX+'<br/>'+this.series.name+': <b>'+this.percentage.toFixed(1)+' %</b>';
        }
      },
      plotOptions: {pie: {allowPointSelect: true,cursor: 'pointer',dataLabels:{enabled: true, format:'{point.percentage:.1f} %'},
				...
				<xsl:variable name="pos" select="position()+ 1"/>
					<xsl:choose>
						<xsl:when test="position()= 2">{
							...
							y:<xsl:value-of select="$grafico/value/row[position()=1]/col[position()=$pos]"/>,
							sliced:true,
							selected: true
						},</xsl:when>
  ...
  <xsl:template name="funnel-highcharts">
    ...
    $('#id-'+id).highcharts({
      ...
      tooltip: {
        formatter: function(){
          vX = this.key; vY = this.y;
          return '<span style="color:'+this.point.color+'">\u25CF</span> '+vX+'<br/>'+this.series.name+': <b>'+vY+'</b>';
        }
      },
  ...
  <xsl:template name="tableCharts">
    ...
    <div class="table_graph">
      <table style="width:{$width}%;" class="borderColor" id="{$graph-id}">
        <thead>
          <tr>
          ...
          <xsl:for-each select="$grafico/value/row">
            ...
            <tr>
              <xsl:for-each select="col">
                <xsl:choose>
                  <xsl:when test="position()=1">
                    <td><a y="{.}" class="{$nodegraphe}_{$nod}"><xsl:value-of select="."/></a></td>
                  </xsl:when>
                  <xsl:otherwise>
                    <td class="{$graph-id}" pos="{$nod}"><a class="{$nodegraphe}" rel="{$url}" x="{.}"> <xsl:value-of select="."/> </a></td>
                  </xsl:otherwise>
                </xsl:choose>
              </xsl:for-each>
            </tr>
