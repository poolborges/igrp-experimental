...
<gen:stylesheet xmlns:gen="http://www.w3.org/1999/XSL/Transform" xmlns:xsl="dim-red" version="1.0" exclude-result-prefixes="xsl">
	...
	<gen:template name="gen_chart">
		<gen:for-each select="rows/content/*[@type = 'chart']">
				...
				<gen:choose>
					...
					<gen:when test="$type!='hidden' and $type!='separatorlist' and $type!='separatordialog' and $type!='formlist' and $type!='list' and $type!='iframe' ">
						<gen:choose>
							...
							<!-- date.form -->
							<gen:when test="$type='date'">
								<div class="form-group col-sm-3" item-name="{name()}" item-type="{$type}">
							...
							<gen:when test="contains($type,'select')">
								...
								<div class="form-group col-sm-3 " item-name="{name()}" item-type="select">
									<gen:call-template name="isRequiredClass">
										<gen:with-param name="isRequired" select="$isRequired"/>
									</gen:call-template>
									<label for="{$nameAttr}">
							...
							<!-- checkbox.form -->
							<gen:when test="$type='checkbox'">
								<div class="col-sm-3" item-name="{name()}" item-type="{$type}">
