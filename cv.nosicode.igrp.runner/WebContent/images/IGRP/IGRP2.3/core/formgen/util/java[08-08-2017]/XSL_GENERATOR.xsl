...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
		...
		<xsl:if test="not(rows/plsql/package_instance/text())">
		...
		<xsl:call-template name="uppercase">
			<xsl:with-param name="value">
				<xsl:value-of select="substring(rows/plsql/package_instance,1)" />
			</xsl:with-param>
		</xsl:call-template>
		<xsl:text>.</xsl:text>
		...
		<xsl:value-of select="$enter" />
		<xsl:text>import nosi.helper.*;</xsl:text>
		<xsl:value-of select="$enter" />
		...
		<xsl:text>.</xsl:text>
		<xsl:value-of select="substring(rows/plsql/package_db,2)" />
		...
		<xsl:for-each select="rows/content/*[@type='chart']/label/*">
			<xsl:text>public Field </xsl:text>
			<xsl:value-of select="." />
			<xsl:text>;</xsl:text>
			<xsl:value-of select="$enter" />
		...
		<xsl:for-each select="rows/content/*[@type='table'] | rows/content/*[@type='chart']">
			<!-- <xsl:value-of select="$tab" /> -->
		...
		<xsl:value-of select="$tab" />
		<xsl:value-of select="$tab" />
		<xsl:text>view = new nosi.app.</xsl:text>
		<xsl:call-template name="uppercase">
			<xsl:with-param name="value">
				<xsl:value-of select="substring(rows/plsql/package_instance,1)" />
			</xsl:with-param>
