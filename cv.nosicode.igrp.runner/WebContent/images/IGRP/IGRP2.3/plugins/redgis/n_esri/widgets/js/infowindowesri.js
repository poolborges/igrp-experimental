    ...
    jQ.fn.InfoWindowEsri= function() {
		...
		_iw.queryFeature = function(idx){
			if(idx < _iw.Layers.length){
				...
				if(LayersManager.getLayerVisibility(_layerid)){
					...
					var layerType = LayersManager.getLayer(_layerid).geometryType;
					var qGeom = (layerType == 'esriGeometryPolyline' || layerType == 'esriGeometryPoint') ? pointToExtent(map,_iw.MappPoint,10) : _iw.MappPoint;
		...
		_iw.setMenuActive   = function(layerid){
			try{
				...
				var colorString = selectionColor ? selectionColor.r+','+selectionColor.g+','+selectionColor.b : '0,0,0';
		...
		var pointToExtent = function(map, point, toleranceInPixel) {
			...
			var _return = new esri.geometry.Extent( 
