	...
	if(!Mapp){
		try{
			Mapp = new Object({
				...
				project : function(p){
					//console.log(esri.geometry.geometryEngine)
					var outSR  = new esri.SpatialReference(p.spatialReference)
					//var params = new new esri.tasks.ProjectParameters();

					Mapp.geometryService.project(p.geometries, outSR, function(projectedPoints){
				...
				settings:{
					basemap:{
						...
						set:function(bm,center,zoom){	
							...
							switch(bm.type){
								case 'ESRI':
									_layer= new esri.layers.ArcGISTiledMapServiceLayer(bm.url);

								break;
								case'Feature Access':
									_layer = new esri.layers.ArcGISDynamicMapServiceLayer(bm.url);
								break;
							}
...
function showCoordinates(evt) {
	...
	var mp = map.spatialReference.isWebMercator() ? esri.geometry.webMercatorToGeographic(evt.mapPoint) : evt.mapPoint;
	//console.log( esri.geometry.geographicToWebMercator(evt.mapPoint) )
	//console.log(map.spatialReference.isWebMercator())
	jQ('#mapp-coords-info #mapp-c-x').text(mp.x.toFixed(4));
	jQ('#mapp-coords-info #mapp-c-y').text(mp.y.toFixed(4));
