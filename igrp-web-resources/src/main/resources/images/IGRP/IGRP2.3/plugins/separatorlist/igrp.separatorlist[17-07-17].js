$.fn.separatorList = function(o){
	...
	if(this[0]){
		...
		var appendToTable = function(values,sl){
			...
			var	rowIndex   = sle.indexRow ? sle.indexRow*1 : $(sl).attr('row-index')*1,
				...
				action     = sle.action ? sle.action : $(sl).attr('row-action'),
		...
		var varRowEdit = {
			set : function(p){
				...
				localStorage.setItem(p.sl.name, JSON.stringify(slEstorage));
			},
			reset : function(sl){
			...
			$.each(fields,function(i,f){
				...
				if(fname){
					...
					//ALL OTHER INPUTS HANDLER
						...
						if (ftype == 'file' && val) {
							var file = field.clone(true);
							field.removeAttr('id required').attr('name',fname+'_fk').hide();
							file.insertBefore(field);
